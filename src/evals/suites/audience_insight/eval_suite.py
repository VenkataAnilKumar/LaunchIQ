"""Audience Insight agent evaluation suite."""
...
async def run_suite() -> dict:
    """Run Audience Insight eval suite against test cases and expected outputs."""
    evaluator = Evaluator()
    test_cases = json.loads((SUITE_DIR / "test_cases.json").read_text())
    expected = json.loads((SUITE_DIR / "expected_outputs.json").read_text())
    return await evaluator.run_suite("audience_insight", test_cases, expected)
